...
public class StoryService : IStoryService
{
    ...
    public async Task<List<StoryDto>> GetFeedStoriesAsync(Guid userId)
    {
        ...
        // Chỉ lấy bạn bè đã Accepted — tuyệt đối không lấy Pending
        var acceptedFriendIds = await _context.Friendships
            .Where(f =>
                f.Status == "Accepted" &&
        ...
        // Loại bỏ những người đã Blocked (cả 2 chiều)
        ...
        // friendIds = bạn bè Accepted, trừ Blocked
        var friendIds = acceptedFriendIds
            .Except(blockedIds)
            .ToList();

        // Luôn thêm bản thân — story của mình luôn hiện
    ...
    private static StoryDto MapToDtoExpr(Story s) => new()
    {
        Id          = s.Id,
